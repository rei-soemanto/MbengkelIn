-- Customer is liable for an accepted offer: cancelling an On Progress order now
-- settles like a completion (charge the customer, pay the bengkel the agreed
-- price) instead of refunding. A not-yet-accepted (To Do) cancel still just
-- releases the hold (no charge).
...
begin
  ...
  if NEW.status = 'Cancelled' and OLD.status <> 'Cancelled' then
    if OLD.status = 'On Progress' then
      -- Liable for the accepted offer: charge the customer and pay the bengkel.
      update public.users
        set balance = balance - coalesce(OLD.price,0),
            held_balance = greatest(0, held_balance - coalesce(OLD.price,0))
        where id = NEW.customer_id;
      select provider_uid into prov from public.bengkels where id = OLD.bengkel_id;
      if prov is not null then
        update public.users
          set balance = balance + coalesce(OLD.price,0),
              pending_balance = greatest(0, pending_balance - coalesce(OLD.price,0))
          where id = prov;
      end if;
    else
      -- Not yet accepted (To Do): just release the hold, no charge.
