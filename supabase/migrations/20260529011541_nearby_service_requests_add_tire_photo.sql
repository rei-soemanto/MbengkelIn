...
CREATE OR REPLACE FUNCTION public.nearby_service_requests(p_lat double precision, p_lon double precision, p_radius_m double precision DEFAULT 5000)
 RETURNS TABLE(id uuid, customer_id uuid, customer_name text, service_type text, description text, is_emergency boolean, latitude double precision, longitude double precision, price bigint, status text, tire_count integer, photo_url text, created_at timestamp with time zone, distance_m double precision)
 LANGUAGE sql
 SECURITY DEFINER
