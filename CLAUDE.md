...
| `service_type`                              | enum `ServiceType`   | "Ban Gembos", "Ban Pecah", "Aki Kering", "Mogok / Mesin Mati", "Ganti Ban Serep", "Rantai Motor Lepas", "Mesin Overheat" — **Postgres enum**: new values need an `alter type ... add value` migration, not just a Swift `ServiceType` case; removing a value needs a type-swap migration (Postgres has no `drop value`) |
| `description`                               | text?                |                                                                                               |
| `is_emergency`                              | bool                 |                                                                                               |
